...
AGASCharacter::AGASCharacter()
{
	...
	//JAQ: Setup Ability system component
...
void AGASCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//JAQ: Also call this here in case the initialisation order is wrong
	BindInputToAbilitySystemComponent();
}

//JAQ: Overriden method that just returns our component
...
void AGASCharacter::InitializeAttributes()
{
	...
	{
		//JAQ: Some information on the context in which we're applying the effect (e.g. I'm the instigator, and I'm damaging someone else)
		FGameplayEffectContextHandle EffectContextHandle = AbilitySystemComponent->MakeEffectContext();
		EffectContextHandle.AddSourceObject(this);

		//JAQ: Don't quite get the spec tbh, but build it with the default effect we've assigned to this class, and a level, and the context
		...
		//JAQ: If the spec is valid, use the effect to apply changes to an attribute
...
//JAQ: Server must assign abilities
void AGASCharacter::GiveAbilities()
{
	if(HasAuthority() && AbilitySystemComponent)
	{
		...
		{
			//JAQ: As above, define a spec for the ability, with the class, the level, and inputID (e.g. the punch input id), and the source object (optional)
...
//JAQ: Server GAS init
//JAQ: Some concern that possibly shouldn't run InitializeAttributes and InitABilityActorInfo here if also a listen server
void AGASCharacter::PossessedBy(AController* NewController)
{
	Super::PossessedBy(NewController);

	//JAQ: Who owns the GAS, and who's the avatar
	//JAQ: so controller might own it, but the character is the avatar
...
void AGASCharacter::OnRep_PlayerState()
{
	...
	//JAQ: Note that only the server should give abilities

	BindInputToAbilitySystemComponent();
}

//JAQ: This is called from OnRep_PlayerState and SetupPlayerInputComponent as we're not sure on the initialisation order
void AGASCharacter::BindInputToAbilitySystemComponent()
{
	if(AbilitySystemComponent && InputComponent)
	{
		//JAQ: Setup the confirm and canel binds. Some magic looks through and matches our enum up to our inputs
		const FGameplayAbilityInputBinds Binds("Confirm", "Cancel", "EGASAbilityInputID", static_cast<int32>(EGASAbilityInputID::Confirm), static_cast<int32>(EGASAbilityInputID::Cancel));

		//Then bind those to the component
